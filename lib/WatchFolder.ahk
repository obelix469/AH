...
; ==================================================================================================================================
WatchFolder(Folder="E:\Downloads", UserFunc="WFF", SubTree:=False, Watch:=0x03) {
   ...
   If (Folder = TimerID) { ; called by timer
      If (ObjCount:=EventArray.Length()) && !Paused {
         ...
         While (ObjIndex >= 0) && (ObjIndex < ObjCount) {
            FolderName:=WatchedFolders[ObjIndex + 1], D:=WatchedFolders[FolderName]
            If DllCall("GetOverlappedResult", "Ptr", D.Handle, "Ptr", D.OVLAddr, "UIntP", BytesRead, "Int", True) {
               Changes:=[], FNIAddr:=D.FNIAddr,FNIMax:=FNIAddr + BytesRead,OffSet:=0,PrevIndex:=0,PrevAction:=0,PrevName:=""
               Loop {
                  FNIAddr+= Offset
                  OffSet:=NumGet(FNIAddr + 0, "UInt"),Action:=NumGet(FNIAddr + 4, "UInt")
                  ...
                  PrevAction:=Action,PrevName:=Name
